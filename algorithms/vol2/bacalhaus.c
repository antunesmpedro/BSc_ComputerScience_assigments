...
GRAFO* ler_construir_grafo() {
  int n_nos, n_ramos, i, start, end, temperatura, custo;

  scanf("%d %d", &n_nos, &n_ramos);

  GRAFO* g = new_graph(n_nos);

  for (i=0; i<n_ramos; i++) {
    scanf("%d %d %d %d", &start, &end, &temperatura, &custo);
    insert_new_arc(start, end, temperatura, custo, g);
    insert_new_arc(end, start, temperatura, custo, g);
  }
...
int main() {
  GRAFO *g;
  int n_nos_percurso, i, start, end;
  int temp_min=INT_MAX, temp_max=INT_MIN;
  ARCO * arc;

  g = ler_construir_grafo();

  scanf("%d", &n_nos_percurso);
  while (n_nos_percurso != 0) {
    scanf("%d", &start);
    temp_min=INT_MAX;
    temp_max=INT_MIN;
    for (i = 1; i < n_nos_percurso; i++) {
      scanf("%d", &end);
      arc = find_arc(start,end,g);
      if (arc != NULL){
        if (VALOR1_ARCO(arc) < temp_min)
          temp_min = VALOR1_ARCO(arc);
        if (VALOR1_ARCO(arc) > temp_max)
          temp_max = VALOR1_ARCO(arc);
      }
      start = end;
    }
    printf("%d %d\n", temp_min, temp_max);
