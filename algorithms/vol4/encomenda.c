...
GRAFO* ler_construir_grafo(int l_min, int c_min, int a_min) {
  int novo_i, novo_j;
  int i, j, comp, larg, alt;
  int origem, destino;
  GRAFO* g;
  
  g = new_graph(MAXVERTS);
  for (i=1; i<=MAXVERTS+1; i++) 
    nos[i] = 0;
  ...
  scanf("%d", &i);
  while (i != -1) {
    scanf("%d %d %d %d", &j, &larg, &comp, &alt);
    if (larg >= l_min && comp >= c_min && alt >= a_min) {
      novo_i = procura(i);
      novo_j = procura(j);

      if (find_arc(novo_i,novo_j,g) == NULL){
        insert_new_arc(novo_j,novo_i,comp,g);
        insert_new_arc(novo_i,novo_j,comp,g);
      }
    }
    scanf("%d", &i);
  }
  NUM_VERTICES(g) = cont_nos;
  return g;
}

int dijkstra(int start, int end, int max, GRAFO* g) {
  HEAPMAX* q;
  int v,w,i, minimo;
  ARCO* cur_adj;
  ...
  while ( !heap_isEmpty(q) ) {
    ...
    if ( cap[v] == 0 || v == end ) break;

    cur_adj = ADJS_NO(v,g);
    while ( cur_adj != NULL ) {
      w = EXTREMO_FINAL(cur_adj);
      minimo = min(VALOR_ARCO(cur_adj),cap[v]);
      if ( cap[w] < minimo ) {
        //printf("%d -> %d\n", v, w);
        cap[w] = minimo;
...
int main(){
  int l_min, l_max, c_min, c_max, a_min;
  GRAFO* g;
  int val;  
  scanf("%d %d %d %d %d", &l_min, &l_max, &c_min, &c_max, &a_min);

  g = ler_construir_grafo(l_min, c_min, a_min);
  val = dijkstra(1,2,c_max,g);
  printf("%d\n",val);
