...
GRAFO* ler_construir_grafo() {
  ...
  while (r) {
    scanf("%d %d %d", &x, &y, &lugares);
    if (find_arc(x,y,g)==NULL)
      insert_new_arc(x,y,lugares,g);
    r--;
  }
  return g;
}
...
void dijkstra(int start, int max, GRAFO* g) {
  int v,w,i;
  ARCO* cur_adj;
  HEAPMAX* q;
  ...
  for (i=1;i<=NUM_VERTICES(g);i++)
    capacidade[i] = 0;
  capacidade[start] = max;
  q = build_heap_max(capacidade,NUM_VERTICES(g));
  while (!heap_isEmpty(q)) {
    ...
    if (capacidade[v] != 0) {
      while (cur_adj) {
        ...
        if (capacidade[w] < min(VALOR_ARCO(cur_adj),capacidade[v])){
          capacidade[w] = min(VALOR_ARCO(cur_adj),capacidade[v]);
          increaseKey(w,capacidade[w],q);
        }
...
int main() {
  int min_elem, origem, max_elem;
  GRAFO* g;
  int i, encontrou = 0;

  scanf("%d %d %d", &min_elem, &origem, &max_elem);

  g = ler_construir_grafo();

  dijkstra(origem,max_elem,g);

  for (i=1; i<=NUM_VERTICES(g); i++) {
    if (capacidade[i] >= min_elem && i!=origem) {
      printf("Destino %d: %d\n", i, capacidade[i]);
      encontrou = 1;
    }
  }
