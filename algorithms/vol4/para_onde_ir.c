...
GRAFO* ler_construir_grafo() {
  ...
  while (n_ramos) {
    scanf("%d %d %d %d", &i, &j, &lugares, &preco);
    if (find_arc(i,j,g) == NULL)
      insert_new_arc(i,j,lugares,preco,g);
...
int dijkstra(int start, int elementos, int mont_max, GRAFO* g) {
  int res = 0, i, v, w;
  HEAPMIN* q;
  ARCO* cur_adj;
  ...
  while (heap_isEmpty(q) == 0) {
    v = extractMin(q);
    cur_adj = ADJS_NO(v,g);
    if (preco[v] != INT_MAX) {
      while (cur_adj != NULL) {
        w = EXTREMO_FINAL(cur_adj);
        if (VALOR1_ARCO(cur_adj) >= elementos){
          if (preco[v]+VALOR2_ARCO(cur_adj) < preco[w]) {
            preco[w] = preco[v] + VALOR2_ARCO(cur_adj);
...
int main() {
  GRAFO* g;
  int mont_max, n_elementos, origem, i;

  scanf("%d %d %d", &n_elementos, &origem, &mont_max);

  g = ler_construir_grafo();

  int res = dijkstra(origem,n_elementos,mont_max,g);
