...
GRAFO* ler_construir_grafo() {
  GRAFO* g;
  char palavra1[MAXVERTS+1];
  char palavra2[MAXVERTS+1];
  int size, i, x, y;

  g = new_graph(MAXVERTS);
  scanf("%s", palavra1);
  scanf("%s", palavra2);  
  while (strcmp(palavra2,"#") != 0){
    size = strlen(palavra1);
    for (i=0; i<size; i++) {
      x = palavra1[i];
      y = palavra2[i];
      if (x != y){
...
BOOL analisa_caminho_bfs(char caminho[], GRAFO* g) {
  int size, i, x, y, inicio;
  int visitados[MAXVERTS], v, w;
  QUEUE* q;
  ARCO* arc;

  size = strlen(caminho);
  x = inicio = caminho[0];
  //BFS com update se contem nodes no caminho lido
  for (i = 1; i<size; i++) {
    y = caminho[i];
    if (find_arc(x,y,g) == NULL)
      return 0;
...
int main() {
  GRAFO* g;
  char caminho[MAXVERTS];
  g = ler_construir_grafo();

  scanf("%s", caminho);
  while (strcmp(caminho,"#") != 0) {
    if (analisa_caminho_bfs(caminho,g) == TRUE)
      printf("Sim\n");
    else
      printf("Nao\n");
    scanf("%s", caminho);
