...
GRAFO* ler_construir_grafo() {
	...
	scanf("%d %d %d", &n_nos, &n_ramos, &custo_man);
	g = new_graph(n_nos);
	while (n_ramos) {
		scanf("%d %d %d", &x, &y, &rendimento);
		if (find_arc(x,y,g) == NULL){
			insert_new_arc(x,y,rendimento,g);
			insert_new_arc(y,x,rendimento,g);
		}
		n_ramos--;
	}
	return g;
...
BOOL prim(int start, GRAFO* g) {
	HEAPMAX* q;
	ARCO* cur_adj;
	dist[start] = 0;
	q = build_heap_max(dist, NUM_VERTICES(g));
	while (heap_isEmpty(q) == 0) {
		...
		while (cur_adj != NULL) {
			...
			if (ok[w] == FALSE && VALOR_ARCO(cur_adj) > dist[w]) {
				dist[w] = VALOR_ARCO(cur_adj);
				pai[w] = v;
...
int main() {
	...
	if (prim(1,g) == TRUE) {
		valor_bruto -= (NUM_VERTICES(g)-1) * custo_man;
